...
def _extrair_coordenadas_relativas(posicao_str: str, viewport_w: int, viewport_h: int) -> dict:
    try:
        ...
        return {
            "x_pct": round((x + w_el / 2) / viewport_w, 4), 
            "y_pct": round((y + h_el / 2) / viewport_h, 4)
        }
    except Exception:
...
async def _injetar_em_contexto(contexto):
    # CORREÇÃO: Usando \\d em vez de \d para resolver o SyntaxWarning do Python 3.12+
    script_radar = """() => {
        ...
        const getBestSelector = (el) => {
            ...
            for (let i = 0; i < 5; i++) {
                if (!current) break;
                if (current.getAttribute('data-testid')) return `[data-testid='${escapeStr(current.getAttribute('data-testid'))}']`;
                if (current.getAttribute('aria-label')) return `[aria-label='${escapeStr(current.getAttribute('aria-label'))}']`;
                if (current.id && !current.id.match(/^[\\d\\-_]/) && !current.id.match(/ng-|mat-|cdk-/)) return `[id='${escapeStr(current.id)}']`;
                current = current.parentElement;
            }
            ...
            if (isEditable && el.getAttribute('contenteditable') === 'true') return `[contenteditable='true']`;
            
            let text = el.innerText?.trim().replace(/\\n/g, ' ') || '';
            if (text && text.length > 1 && text.length < 50) return `text="${escapeStr(text)}"`;
        ...
        const getFrameIdentifier = () => {
            ...
            try {
                if (window.top && window.location.href !== window.top.location.href) return window.location.href.split('/').pop().split('?')[0] || 'iframe';
            } catch (e) { return 'iframe_cross_origin'; }
...
async def on_capturar_elemento(source, args):
    ...
    try:
        ...
        screenshot_b64, page_ref, screenshot_bytes = None, None, None
        vp_w, vp_h = 1920, 1080 
...
async def capturar_cliques_na_tela():
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            # CORREÇÃO DO CRASH: O appendChild agora só roda se o document.body já tiver carregado de verdade
            ...
            try:
                await page.evaluate(script_alerta_seguro)
            except Exception as ev_err:
                logger.debug(f"Aviso ao injetar banner visual: {ev_err}")
                pass
...
# ==============================================================
# 🧠 AURA — GERAÇÃO DO ROTEIRO (Assíncrono e Seguro)
# ==============================================================
def _invocar_aura_sync(objetivo_aula: str, log_mapeador: list, contexto_rag: str):
    logger.info("🧠 Acordando a Aura (Vision-First)...")
    try:
        with open("aura_prompt.txt", "r", encoding="utf-8") as f: prompt_sistema = f.read()
    except FileNotFoundError:
        return logger.error("⚠️ aura_prompt.txt não encontrado.")

    lista_para_ia = [{"id_acao": a["id_acao"], "acao": a["acao"], "intencao_semantica": a["intencao_semantica"], "elemento_alvo_resumido": {k: v for k, v in a["elemento_alvo"].items() if k != "screenshot_referencia"}, "valor_input": a["valor_input"]} for a in log_mapeador]
    ...
    try:
        ...
        logger.info("✅ roteiro.json gerado com sucesso!")
    except Exception as e: logger.error(f"Erro ao montar roteiro: {e}")

async def orquestrador_pos_captura(objetivo: str):
    contexto_rag = await buscar_contexto_pinecone(objetivo)
    await asyncio.to_thread(_invocar_aura_sync, objetivo, cliques_capturados, contexto_rag)
...
def iniciar_esteira_de_producao():
    ...
    if not cliques_capturados: 
        return logger.warning("Nenhuma ação capturada. Encerrando.")
        
    asyncio.run(orquestrador_pos_captura(objetivo))
    
    if input("\n🎬 Tudo pronto! Iniciar Motor (main.py)? (S/N)\n> ").strip().upper() == "S":
