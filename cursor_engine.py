...
# ══════════════════════════════════════════════════════════
# CONSTANTES DE ANIMAÇÃO (MUITO MAIS LENTO E SUAVE)
# ══════════════════════════════════════════════════════════
DURACAO_BASE_MS    = 1200  # Era 650
DURACAO_MIN_MS     = 500   # Era 250
DURACAO_MAX_MS     = 2500  # Era 1200
JITTER_PIXELS      = 2.0
...
(function() {
    ...
    Object.assign(cursor.style, {
        ...
        transition: 'top 0.1s ease-out, left 0.1s ease-out, opacity 0.4s ease', // Fade de opacidade
    ...
    // "VÍRUS BENIGNO": Garante a cada 1 seg que nenhum iframe novo carregue a seta nativa
    setInterval(() => {
...
async def instalar_cursor(page) -> bool:
    try:
        ...
        await page.add_style_tag(content="* { cursor: none !important; } html, body { cursor: none !important; }")
        return True
...
def _bezier_cubica(t: float, p0x: float, p0y: float, p1x: float, p1y: float, p2x: float, p2y: float, p3x: float, p3y: float) -> tuple[float, float]:
    mt = 1 - t
    x = (mt**3 * p0x + 3 * mt**2 * t * p1x + 3 * mt * t**2 * p2x + t**3 * p3x)
    y = (mt**3 * p0y + 3 * mt**2 * t * p1y + 3 * mt * t**2 * p2y + t**3 * p3y)
    return x, y

def _gerar_pontos_controle(x0: float, y0: float, x3: float, y3: float) -> tuple[float, float, float, float]:
    dx, dy = x3 - x0, y3 - y0
    distancia = math.hypot(dx, dy)
    if distancia < 1: return x0, y0, x3, y3
...
async def mover_cursor_humanizado(page, x_fim: float, y_fim: float, duracao_ms: Optional[int] = None) -> None:
    try:
        pos = await page.evaluate("() => ({ x: window._cursorX ?? window.innerWidth/2, y: window._cursorY ?? window.innerHeight/2 })")
...
async def obter_coords_acao(page, acao_tec: dict) -> Optional[dict]:
    seletor = acao_tec.get("seletor_css") or acao_tec.get("seletor")
